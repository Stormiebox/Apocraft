...
--Players keep favorites when dismantling items
...
--Change parameter in lighter zippo
...
--Recycling items for strength xp
...
function Recipe.OnCreate.TryMakeGlass(items, result, player)
    ...
    if ZombRand(0, 9) < finalChance then
        ...
        player:getInventory():AddItems("ShardsOfGlass", 4)
        player:getXp():AddXP(Perks.Strength, 4)
    end
end
